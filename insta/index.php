...
<style type="text/css">
	...
	.item .title .pic_nouser{
		position: absolute;
		top:10px;
		left:<?php if($is_admin) echo "60"; else echo "10"; ?>px;
		width: 40px;
		height: 40px;
		background-size: 40px 40px;
	}
...
<div style="display:none">
	<form id="sync" action="http://www.wiggle-wiggle.com/syncArtshare.php" method="post" enctype="multipart/form-data" style="display:none">
		<textarea name="data"><?php
			$result = sql_query("select * from g5_insta order by in_datetime desc");
			while($row=sql_fetch_array($result)){
				echo "$row[in_id]^^$row[in_profile]^^$row[in_img]^^$row[in_unuse]||";
			}
		?></textarea>
	</form>
</div>
...
<?php if($is_admin) { ?>
	<a href='./keep.php'><div class="btnUpdate"><div class="inner">[관리] 보류 중인 게시글 확인하기</div></div></a>
<?php } ?>
<?	
	$row = sql_fetch("select count(*) as cnt from g5_insta where in_unuse='$unuse'");
?>
<?php if (!G5_IS_MOBILE) { ?><Br>
	<div class="btnUpdate"><div class="inner">'#위글위글'이 태그된 <?=$row[cnt]?>개의 후기</div></div>
	<?php } ?>
<div id="ma_con">
<?php
