...
function get_banner($position,$w,$h)
{
	$link01 = "";
	$link02 = "";
	$new_win = "";
	
	$row = sql_fetch("select * from g5_shop_banner where bn_position='$position' order by bn_id desc");
	
	if($row[bn_new_win])
		$new_win = "target='_blank'";
		
	if($row[bn_url] && $row[bn_url] != "http://"){
		$link01 = "<a href='$row[bn_url]' $new_win>";
		$link02 = "</a>";
	}
	$str = "";
	...
	if($row[bn_id])
		$str = $link01."<img src='".G5_URL."/data/banner/$row[bn_id]' width='$w' $height_str border='0' alt='$position'>".$link02;
