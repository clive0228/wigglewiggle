...
function setOpen(bool){
	if (_isOpen == true) {
		_domBaseBg.stop().animate({ height : 93, opacity : _defaultOpacity }, 600, "easeInOutExpo");
		...
		$("#redLine").animate({ opacity : 0 }, 200);
	} else {
		
		_domBaseBg.stop().animate({ height : _openHeight+93 , opacity : 1 }, 600, "easeOutExpo");
